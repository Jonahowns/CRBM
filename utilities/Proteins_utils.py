...
def seq2num(string):
    if type(string) == str:
        return np.array([dnadict[x] for x in string])[np.newaxis,:]
    elif type(string) ==list:
        return np.array([[dnadict[x] for x in string_] for string_ in string])


def num2seq(num):
    return [''.join([dna[x] for x in num_seq]) for num_seq in num]
